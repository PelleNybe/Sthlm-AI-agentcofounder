...
export async function processSlackJob(job: { id?: string; data: SlackTaskNotificationPayload }) {
  console.log(`Processing Slack notification job ${job.id || 'N/A'} for task ${job.data.taskId || 'N/A'}`);
  try {
    const result = await sendSlackNotification(job.data);
    console.log(`Successfully sent Slack notification for job ${job.id || 'N/A'}`);
    return result;
  } catch (error: any) {
    console.error(`Failed to process Slack notification job ${job.id || 'N/A'}: ${error.message}`);
    throw error;
  }
}
